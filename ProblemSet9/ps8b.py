...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for trial in range(numTrials):
        viruses = []
        virusTotalList = [float(0)] * 300
        virusResistList = [float(0)] * 300
        ...
        while step < 149:
            tp.update()
            virusTotalList[step] += tp.getTotalPop()
            virusResistList[step] += tp.getResistPop(['guttagonol'])
            step += 1
    ...
    avgVirusTotalList = [float(x) for x in virusTotalList]
    avgVirusResistList = [float(x) for x in virusResistList]
