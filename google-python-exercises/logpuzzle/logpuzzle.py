...
def read_urls(filename):
  ...
  f = open(filename, 'r')
  urls = re.findall(r'GET\s(\S+puzzle\S+)\s', f.read())
  f.close
  for url in urls:
    results1.append("http://" + host + url)
...
def download_images(img_urls, dest_dir):
  ...
  for img_url in img_urls:
    ...
    urllib.urlretrieve(img_url, os.path.join(dest_dir, fname))
    i = i + 1
    fname = "img" + str(i) + ".jpg"
    index.write('<img src="' + os.path.join(dest_dir, fname) + '">')
