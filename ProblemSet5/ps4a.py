...
def getWordScore(word, n):
    ...
    for letter in word:
        for key in SCRABBLE_LETTER_VALUES:
            if letter == key:
                result += SCRABBLE_LETTER_VALUES[key]
...
def updateHand(hand, word):
    ...
    work_hand = hand.copy()
    for letter in word:
        for key in work_hand.keys():
            if letter == key:
                work_hand[key] = work_hand[key] - 1
                if work_hand[key] == 0:
                    del work_hand[key]
    return work_hand
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    """
    # BEGIN PSEUDOCODE <-- Remove this comment when you code this function; do your coding within 
    #the pseudocode (leaving those comments in-place!)
