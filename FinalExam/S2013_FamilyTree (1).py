...
class Family(object):
    ...
    def getparent(self,kid):
        if kid in self.names_to_nodes.keys():
            kid_node = self.names_to_nodes[kid]
            return str(kid_node.get_parent())
        else:
            return None
    ...
    def cousin(self, a, b):
        ...
        def getdistance(some):
            listo = []
            while self.getparent(some) != None:
                some = self.getparent(some)
                listo.append(some)
            for entry in listo:
                if entry == "None":
                    listo.remove(entry)
        ...
        c,d = getdistance(a),getdistance(b)
        degree_removed = max(c,d) - min(c,d)

        if a == b or self.getparent(a) == b:
            return (-1,degree_removed)
        else:
            return (min(c,d)-1,degree_removed)
...
#t, r = f.cousin("h", "n")
#print "'h' is a", words[t], "cousin", r, "removed from 'n'"
t, r = f.cousin("b", "c")
...
t, r = f.cousin("i", "q")
print "'i' is a", words[t], "cousin", r, "removed from 'q'"
